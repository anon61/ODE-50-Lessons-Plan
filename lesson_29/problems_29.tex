...
\item Given that $\lambda$_{1}$ = 2$ and $\lambda$_{2}$ = -1$ are eigenvalues, find the others
...
\textbf{Problem 29:} $\text{tr}(A) = -3$, so $\lambda$_{2}$ + \lambda$_{3}$ = -5$. With conjugates $a \pm bi$: $2a = -5$, so $\lambda_{2,3} = -2.5 \pm bi$

\textbf{Key Formulas:}
\begin{itemize}
\item Liouville: $W(t) = W(t$_{0}$)e^{\int_{t$_{0}$}$^{t}$ \text{tr}(A(s))ds}$
