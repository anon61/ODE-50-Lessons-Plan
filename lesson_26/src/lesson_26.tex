...
\item Last row: negative coefficients in order from $a_{0}$ to $a_{n-1}$
\end{itemize}
\end{insight}
...
\begin{enumerate}
\item Write the equation in standard form: $y^{(n)} = -a_{n-1}y^{(n-1)} - \cdots - a_{0}y + f(t)$
\item Define state variables: $x_{1} = y, x_{2} = y', \ldots, x_{n} = y^{(n-1)}$
...
&\vdots \\
x_{n-1}' &= x_{n} \\
x_{n}' &= -a_{n-1}x_{n} - \cdots - a_{1}x_2 - a_{0}x_1 + f(t)
\end{align}
