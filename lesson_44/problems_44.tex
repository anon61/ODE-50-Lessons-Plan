...
\begin{enumerate}[resume]
\item Given that $y$_{1}$ = e^{-t}$ solves $(D+1)$^{2}$[y] = 0$, use reduction of order to find $y$_{2}$$.

\item Verify that $te^{3t}$ solves $y'' - 6y' + 9y = 0$ by direct substitution.

\item Show that if $y$_{1}$ = e^{rt}$ solves $L[y] = 0$ where $r$ is a double root, then $y$_{2}$ = te^{rt}$ also solves it.

\item Given $y'' - 4y' + 4y = 0$ has solution $e^{2t}$, find the second solution using $y$_{2}$ = v(t)e^{2t}$.

\item For the equation with characteristic polynomial $(r-a)$^{3}$ = 0$, verify the Wronskian of $e^{at}, te^{at}, t$^{2e}$^{at}$ is nonzero.
\end{enumerate}
...
\begin{enumerate}[resume]
\item Prove that if $r$_{0}$$ is a root of multiplicity $m$ of the characteristic polynomial, then $p(r$_{0}$) = p'(r$_{0}$) = \cdots = p^{(m-1)}(r$_{0}$) = 0$ but $p^{(m)}(r$_{0}$) \neq 0$.
...
Root: $r = 3$ (mult. 2)\\
General solution: $y(t) = (c$_{1}$ + c$_{2t}$)e^{3t}$

\item Char. eq.: $r$^{3}$ + 3r$^{2}$ + 3r + 1 = (r+1)$^{3}$ = 0$\\
Root: $r = -1$ (mult. 3)\\
General solution: $y(t) = (c$_{1}$ + c$_{2t}$ + c$_{3t}$^2)e^{-t}$

\item Char. eq.: $(r-1)$^{4}$ = 0$\\
Root: $r = 1$ (mult. 4)\\
General solution: $y(t) = (c$_{1}$ + c$_{2t}$ + c$_{3t}$^2 + c$_{4t}$^3)e$^{t}$$

\item $(r+2)$^{2}$ = 0$, so $y = (c$_{1}$ + c$_{2t}$)e^{-2t}$\\
$y(0) = c$_{1}$ = 2$\\
$y'(t) = c$_{2e}$^{-2t} - 2(c$_{1}$ + c$_{2t}$)e^{-2t}$\\
$y'(0) = c$_{2}$ - 2c$_{1}$ = -3$\\
$c$_{2}$ = 1$\\
...
$y(t) = (c$_{1}$ + c$_{2t}$)\cos(\sqrt{2}t) + (c$_{3}$ + c$_{4t}$)\sin(\sqrt{2}t)$
\end{enumerate}

\subsection*{Part D: Reduction of Order Applications}

\begin{enumerate}[resume]
\item Let $y$_{2}$ = v(t)e^{-t}$\\
Substituting into $(D+1)$^{2}$[y] = y'' + 2y' + y = 0$:\\
After simplification: $v'' = 0$\\
Thus $v = c$_{1t}$ + c$_{2}$$\\
Second solution: $y$_{2}$ = te^{-t}$

\item $y = te^{3t}$\\
$y' = e^{3t} + 3te^{3t} = (1 + 3t)e^{3t}$\\
$y'' = 3e^{3t} + 3(1 + 3t)e^{3t} = (6 + 9t)e^{3t}$\\
Substitute: $(6 + 9t)e^{3t} - 6(1 + 3t)e^{3t} + 9te^{3t}$\\
$= e^{3t}[6 + 9t - 6 - 18t + 9t] = 0$ \checkmark

\item For double root $r$: $(D-r)$^{2}$[y] = 0$\\
If $y = te^{rt}$: $y' = e^{rt} + rte^{rt}$\\
$y'' = 2re^{rt} + r$^{2te}$^{rt}$\\
$(D-r)$^{2}$[y] = D[(D-r)[te^{rt}]] = D[e^{rt}] = re^{rt} - re^{rt} = 0$ \checkmark

\item $y$_{2}$ = v(t)e^{2t}$\\
$y$_{2}$' = (v' + 2v)e^{2t}$\\
$y$_{2}$'' = (v'' + 4v' + 4v)e^{2t}$\\
Substituting: $e^{2t}[v'' + 4v' + 4v - 4v' - 8v + 4v] = e^{2t} \cdot v'' = 0$\\
$v'' = 0 \Rightarrow v = c$_{1t}$ + c$_{2}$$\\
Second solution: $y$_{2}$ = te^{2t}$

\item $W = \begin{vmatrix} e^{at} & te^{at} & t$^{2e}$^{at} \\ ae^{at} & (1+at)e^{at} & (2t+at$^{2}$)e^{at} \\ a$^{2e}$^{at} & (2a+a$^{2t}$)e^{at} & (2+4at+a$^{2t}$^2)e^{at} \end{vmatrix}$\\
After calculation: $W = 2e^{3at} \neq 0$
\end{enumerate}
...
\begin{enumerate}[resume]
\item If $p(r) = (r - r$_{0}$)$^{m}$ q(r)$ where $q(r$_{0}$) \neq 0$:\\
$p'(r) = m(r - r$_{0}$)^{m-1}q(r) + (r - r$_{0}$)$^{m}$ q'(r)$\\
At $r = r$_{0}$$: $p'(r$_{0}$) = 0$ if $m > 1$\\
Continue differentiating to show $p^{(k)}(r$_{0}$) = 0$ for $k < m$\\
But $p^{(m)}(r$_{0}$) = m! \cdot q(r$_{0}$) \neq 0$

\item Total number of solutions = sum of all multiplicities = degree of polynomial = order of ODE

\item Char. poly: $(r-2)$^{2}$(r+1)(r-3) = 0$\\
$y = (c$_{1}$ + c$_{2t}$)e^{2t} + c$_{3e}$^{-t} + c$_{4e}$^{3t}$\\
...
\item $y(t) = (c$_{1}$ + c$_{2t}$)e^{at}$

\item Bounded as $t \to \infty$ only if $a \leq 0$

\item $y(0) = c$_{1}$ = 1$\\
$y'(t) = c$_{2e}$^{at} + a(c$_{1}$ + c$_{2t}$)e^{at}$\\
$y'(0) = c$_{2}$ + ac$_{1}$ = 0$\\
$c$_{2}$ = -a$\\
