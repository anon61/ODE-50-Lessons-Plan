...
\begin{enumerate}
\item Find eigenvalues $\lambda_{1}, \ldots, \lambda_{n}$ of $A$
\item Find corresponding eigenvectors $\mathbf{v}_{1}, \ldots, \mathbf{v}_{n}
\item Solutions: \mathbf{x}_{i}(t) = e^{\lambda_{i}t}\mathbf{v}_{i}
\item Fundamental matrix: \Phi(t) = [e^{\lambda_{1}t}\mathbf{v}_{1} \mid \cdots \mid e^{\lambda_{n}t}\mathbf{v}_{n}]
\end{enumerate}
\end{method}
...
$$\lambda_{1} = 4, \quad \lambda_{2} = -1$$

\textbf{Step 2:} Find eigenvectors
For $\lambda_{1} = 4$: $\mathbf{v}_{1} = \begin{bmatrix} 2 \\ 3 \end{bmatrix}$
For $\lambda_{2} = -1$: $\mathbf{v}_{2} = \begin{bmatrix} 1 \\ -1 \end{bmatrix}$
...
\Phi(t) = $e^{At}$ = I + At + \frac{A^{2}t^2}{2!} + \cdots
\end{proposition}
