...
If $y_{1}, \ldots, y_{n}$ are $n$ linearly independent solutions, then $y$ is also a solution if and only if:
$$W[y, y_{1}, \ldots, y_{n}](t) = 0$
\end{theorem}
...
Without calculation:
\begin{itemize}
\item If $y_{2} = c \cdot y_{1}$ (proportional), then $W = 0$
\item If solutions have different exponential growth rates, $W \neq 0
\item For $e^{\lambda_{1}$ t}, \ldots, $e^{\lambda_{n}$ t}$ with distinct $\lambda_{i}$: always independent
\end{itemize}
\end{proposition}
