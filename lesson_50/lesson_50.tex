...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{$f(x,y)$, $x_{0}$, $y_{0}$, $h$, $N$ steps}
\KwOut{Numerical solution $(x_{i}, y_{i})$}
...
\For{$i = 0$ to $N-1$}{
    \Switch{Method}{
        ...
        \Case{RK4}{
            $k_{1} \leftarrow f(x,y)$\;
            $k_{2} \leftarrow f(x+h/2, y+hk_{1}/2)$\;
            $k_{3} \leftarrow f(x+h/2, y+hk_{2}/2)$\;
            $k_{4} \leftarrow f(x+h, y+hk_{3})$\;
            $y \leftarrow y + h(k_{1}+2k_{2}+2k_{3}+k_{4})/6$\;
        }
    }
    $x \leftarrow x + h$\;
    Store $(x,y)$\;
