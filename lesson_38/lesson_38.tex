...
For each real eigenvalue $\lambda$_{i}$$ with eigenvector $\mathbf{v}$_{i}$$:
\begin{enumerate}
    \item Draw line through origin in direction $\mathbf{v}$_{i}$$
    \item Add arrows:
    \begin{itemize}
        \item $\lambda$_{i}$ < 0$: arrows point TOWARD origin
        \item $\lambda$_{i}$ > 0$: arrows point AWAY from origin
    \end{itemize}
